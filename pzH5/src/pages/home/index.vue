<template>
  ...
  <van-row @click="getOrder(item)" class="yy-list" v-for="item in homeData.hospitals" :key="item.id" justify="space-around">
    <van-col span="6">
      <van-image
        width="100"
        height="90"
...
// 首页数据
// reactive 不能直接赋值，所以需要给 homeData 添加一个属性，然后给属性赋值，不会丢失响应式
...
.yy-list {
  ...
  border-radius: .1rem;
  overflow: hidden;
  box-shadow: 0 .3rem .8rem 0 rgba(0, 0, 0, 0.04), 0 .3rem .8rem 0 rgba(0, 0, 0, 0.04);
  .yy {
    .yy-name {
      font-size: 1.25rem;
      line-height: 2.1rem;
